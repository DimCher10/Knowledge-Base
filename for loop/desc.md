...
Позволяет выполнить указанный блок команд для каждого элемента _итерируемого_ объекта
...
Таким образом, **итерируемые объекты** (iterables) - объекты, которые могут быть обработаны циклом `for` (поочерёдный проход по элементам)
...
> ! Код, приведенный ниже, **позволяет** изменить элементы обрабатываемых объектов (если эти объекты являются изменяемыми) 
...
Используем `for index, value in enumerate(nums)` (проход по индексам элементов списка)
```python
nums = [111, 456, 503, 802]
#        0    1    2    3 
...
for index, value  in enumerate(nums):
    ...
    if nums[index] % 2 == 0:
        print(value)  # распечатываем индекс числа 
```
Результат:
```
1
...
Используем `for index, value in enumerate(nums)` (проход по индексам списка, что позволит использовать код вида `nums[i] = 1`)
```python
nums = [131, 456, 503, 802]
#        0    1    2    3 
...
for index, value in enumerate(nums):
    # обрабатываем nums[0], nums[1] и т.д.
    if nums[index] % 2 == 0:
        nums[index] = 1  # изменяем элемент списка  
